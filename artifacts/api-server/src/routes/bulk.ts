...
// Phase 3: accepts either entityIds (numeric vault_entry ids, unchanged) or
...
router.post("/projects/bulk-enroll", requireAuth, sensitiveWriteLimiter, async (req, res): Promise<void> => {
  const userId = req.user!.userId;
  const { projectId, entityIds, entitySerials, accountData } = req.body as {
    projectId: number; entityIds?: unknown[]; entitySerials?: unknown[]; accountData?: Record<string, unknown>;
  };

  if (!projectId) { res.status(400).json({ error: "projectId required" }); return; }

  const safeEntityIds = new Set(safeIds(entityIds ?? []));
  ...
  for (const entityId of safeEntityIds) {
    try {
      await db.execute(sql`
        INSERT INTO project_enrollments (project_id, vault_entry_id, user_id, account_data)
        VALUES (${projectId}, ${entityId}, ${userId}, ${accountDataJson})
